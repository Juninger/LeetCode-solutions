...
public class LC_746_MinCostClimbingStairs {
    ...
    public int minCostClimbingStairs2(int[] cost) {
        ...
        return Math.min(minCost2(cost, n - 1, memo), minCost2(cost, n - 2, memo));
    }

    // recursively calculate cost to reach i:th step
    public int minCost2(int[] cost, int index, int[] memo) {
        ...
        memo[index] = cost[index] + Math.min(minCost2(cost, index - 1, memo), minCost2(cost, index - 2, memo));
    ...
    public int minCostClimbingStairs3(int[] cost) {
        ...
        // start recursion to calculate cost of reaching the top ("index" n)
        return Math.min(minCost3(cost, n - 1), minCost3(cost, n - 2));
    }

    // recursively calculate cost to reach i:th step
    public int minCost3(int[] cost, int index) {
        if (index == 0) return cost[0]; // cost of first step
        if (index == 1) return cost[1]; // cost of second step

        // cost for i:th step == cost of current step + the MIN cost of reaching the previous two steps
        return cost[index] + Math.min(minCost3(cost, index - 1), minCost3(cost, index - 2));
    }
}
